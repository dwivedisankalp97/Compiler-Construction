...
*/
  ...
  struct grammar_one_function_spill :
    pegtl::must<
      seps,
      Function_rule,
      seps,
...
*/
  ...
  template<> struct action < Instruction_stack_arg_rule > {
    template< typename Input >
      static void apply( const Input & in, Program & p){
      auto currentF = p.functions.back();
      auto i = new Instruction_stack_arg();
      i->M = parsed_items.back();
      parsed_items.pop_back();
      i->left = parsed_items.back();
      parsed_items.pop_back();
...
Program parse_file (char *fileName){
  ...
  Program parse_function_file (char *fileName){
     ...
     * Check the grammar for some possible issues.
     */
    pegtl::analyze< grammar_one_function >();
    ...
    file_input< > fileInput(fileName);
    Program f;
    parse< grammar_one_function, action >(fileInput, f);
  ...
  Program_spill parse_spill_file (char *fileName){
     ...
     * Check the grammar for some possible issues.
     */
    pegtl::analyze< grammar_one_function_spill >();
    ...
    file_input< > fileInput(fileName);
    Program p;
    parse< grammar_one_function_spill, action >(fileInput, p);
    Item new_var = parsed_items.back();
    parsed_items.pop_back();
    Item spill_var = parsed_items.back();
    parsed_items.pop_back();
    Program_spill s = Program_spill(p.functions.back(),spill_var,new_var);
