...
*/
  ...
  struct Instruction_call_rule:
    pegtl::seq<
      pegtl::sor<
        pegtl::seq<
          pegtl::sor<
            pegtl::seq<pegtl::at<str_array>, str_array>,
            pegtl::seq<pegtl::at<str_print>, str_print>,
            pegtl::seq<pegtl::at<str_allocate>, str_allocate>
          >,
          seps,
          pegtl::one< '(' >,
          seps,
  ...
  template<> struct action < Instruction_define_rule > {
    template< typename Input >
      ...
      // cout<<parsed_items.size()<<endl;
      // for(auto v1lb : parsed_items){
      //   cout<<v1lb.labelName<<endl;
      // }
      i->vars.push_back(parsed_items.back());
      parsed_items.pop_back();
      parsed_items.pop_back();
      while(numOfNames > 0){
        // cout<<numOfNames<<endl;
        // cout<<parsed_items.back().labelName<<endl;
        i->vars.push_back(parsed_items.back());
        parsed_items.pop_back();
        numOfNames--;
      }
      std::reverse(i->vars.begin(),i->vars.end());
  ...
  template<> struct action < Instruction_if_rule > {
    template< typename Input >
      ...
      i->flabel = parsed_items.back();
      parsed_items.pop_back();
      i->tlabel = parsed_items.back();
      parsed_items.pop_back();
  ...
  template<> struct action < scope_start_rule > {
    template< typename Input >
      static void apply( const Input & in, Program & p){
      auto currentF = p.functions.back();
      auto i = new Instruction_scope_start();
      currentF->instructions.push_back(i);
    }
  }; 
  ...
  template<> struct action < scope_end_rule > {
    template< typename Input >
      static void apply( const Input & in, Program & p){
      auto currentF = p.functions.back();
      auto i = new Instruction_scope_end();
      currentF->instructions.push_back(i);
    }
  }; 
