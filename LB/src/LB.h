...
namespace LB {
  ...
  enum IType {none,ret,assign,load,assign_cmp_aop,\
    br_var,labelType,call,assign_call,contins,breakins,array,\
    store,br,ret_t,define,array_dest,array_src,new_array,new_tuple,length,ifins,whileins,scope_start,scope_end};
  ...
  string tostring(){
    return "NOt done define";//type.labelName + " " + var.labelName + "\n";
  }
  };
  ...
  string tostringIR(){
    string s = "call ";
    if(fname.labelName != "array-error" && fname.labelName != "print" && fname.labelName != "allocate")
      s += ":";
    s += fname.labelName;
    s+=" (";
  ...
  IType iType(){
    ...
    Item lc,rc,tlabel,flabel,operation;
  ...
  string tostring(){
    return  "while NOT DONE\n";
  }
  };

  struct Instruction_scope_start : Instruction{
  IType iType(){
    return scope_start;
  }
  string tostring(){
    return  "scope start NOT DONE\n";
  }
  };

  struct Instruction_scope_end : Instruction{
  IType iType(){
    return scope_end;
  }
  string tostring(){
    return  "scope end NOT DONE\n";
  }
  };
